    ...
    <div className="min-h-screen bg-gradient-to-r from-indigo-100 via-blue-100 to-pink-100 p-4">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-center mb-6 gap-4">
        <div className="flex items-center gap-4">
          ...
          />
          <div>
            <h1 className="text-3xl md:text-4xl font-bold text-gray-800 animate-pulse">
              Fitness Forge
            </h1>
            <p className="text-sm text-yellow-500">Elite fitness planning & transformation</p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-4">
          <div className="bg-white shadow-md rounded-full px-4 py-2 flex items-center gap-2">
            ...
            <span className="font-semibold text-sm sm:text-base break-all animate-pulse">{username}</span>
          </div>
          <button
            onClick={handleLogout}
            className="bg-black text-white px-4 py-2 rounded-xl hover:bg-gray-800 transition-all text-sm"
          >
            ⎋ Logout
          </button>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 mb-6 animate-pulse">
        ...
        <StatCard title="Sets" icon="📈" color="text-purple-600" count={myExercises.length * 3} />
        <StatCard title="Duration" icon="⏱️" color="text-green-600" count={`${myExercises.length * 5}m`} />
      </div>
      ...
      <div className="flex flex-col sm:flex-row items-center gap-4 mb-6">
        <input
          ...
          className="flex-1 px-4 py-2 rounded-xl shadow focus:outline-none w-full"
        />
        <button className="bg-yellow-400 hover:bg-yellow-500 text-white px-4 py-2 rounded-xl font-bold w-full sm:w-auto">
          📁 Save Name
        </button>
      </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-4 mb-10">
        ...
        <ModalBtn label="⬇️ Export" className="bg-green-500" onClick={() => setShowModal("export")} />
      </div>
      ...
      <div className="bg-white rounded-2xl shadow p-6">
        ...
        {myExercises.length === 0 ? (
          <div className="text-center py-10">
            <div className="text-5xl text-gray-300 mb-4 animate-bounce">🏋️</div>
            <p className="text-xl font-semibold text-gray-700">No exercises in your plan yet</p>
            <p className="text-gray-500">Add some exercises to get started with your elite workout!</p>
          </div>
        ) : (
